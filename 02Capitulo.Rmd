...
Entonces
$$
\nabla \times (M \vec{A}) = \vec{0}
$$
Además,
$$
M \vec{A} \cdot \left( \nabla \times (M \vec{A}) \right) = 0
$$
es decir,
$$
...
$\textbf{Obs \#2:}$ 
$\vec{A} \cdot \left( \nabla \times \vec{A} \right) = 0$ implica
...
La expresión (1) es integrable si existe una función $\mathcal{M}(x,y,z)$, conocida como factor integrable, tal que:
$$
\mathcal{M}(x,y,z) \left( P(x,y,z) \, dx + Q(x,y,z) \, dy + R(x,y,z) \, dz \right)
$$
es una diferencial exacta, es decir,
$$
\mathcal{M}(x,y,z) \left( P(x,y,z) \, dx + Q(x,y,z) \, dy + R(x,y,z) \, dz \right) = d(U(x,y,z))
$$
...
$$
\frac{df}{dx}, \frac{df}{dy},  \frac{df}{dz}=\mu(x,y,z)(P,Q,R)
...
considerar una variable como parametro constante sin perdida de generalidad, se elige z constante , entonces 
$$
dz=0
$$

de esta droma se tiene :
...
buscamos un factor integrante $\mu (x,y,z)$ tal que 
...
La ecuacion diferencia parciales se escribe como :
$$
 dG + Kdy =0
$$
...
$F_p \frac{\partial p}{\partial x} + F_q \frac{\partial p}{\partial y} = - (F_x + F_u p)$
